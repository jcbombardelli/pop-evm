...
use frame_support::{
	decl_error, decl_event, decl_module, decl_storage, ensure, traits::FindAuthor, traits::Get,
};
...
impl<T: Trait> Module<T> {
	fn store_block(n: T::BlockNumber) {
		...
		for t in &transactions {
			...
			if let Some(status) = TransactionStatuses::get(transaction_hash) {
				Transactions::insert(transaction_hash, (hash, status.transaction_index));
			}
		}
	...
	pub fn transaction_by_block_hash_and_index(
		hash: H256,
		index: u32,
	) -> Option<(ethereum::Transaction, ethereum::Block, TransactionStatus)> {
	...
	pub fn block_transaction_statuses(block: &Block) -> Vec<Option<TransactionStatus>> {
		block
			.transactions
			.iter()
			.map(|transaction| {
	...
	> {
		if let Some(block_hash) = block_hash {
			<Module<T>>::block_logs(block_hash, address, topic)
			...
			if from_block >= to_block {
				for number in from_block..to_block {
					...
					if <BlockNumbers<T>>::contains_key(block_number) {
						...
						output.extend(
							<Module<T>>::block_logs(hash, address.clone(), topic.clone()).unwrap(),
						)
					}
				}
				Some(output)
