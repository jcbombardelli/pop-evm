...
use frontier_rpc_core::types::{
	Block, BlockNumber, BlockTransactions, Bytes, CallRequest, EthAccount, Filter, Index, Log,
	Receipt, Rich, RichBlock, SyncStatus, Transaction, VariadicValue, Work,
};
use frontier_rpc_core::EthApi as EthApiT;
use frontier_rpc_primitives::{ConvertTransaction, EthereumRuntimeApi, TransactionStatus};
...
impl<B, C, SC, P, CT, BE> EthApi<B, C, SC, P, CT, BE>
where
			...
			match number {
				BlockNumber::Hash { hash, .. } => {
					if let Ok(Some(block)) = self
						.client
						.runtime_api()
						.block_by_hash(&BlockId::Hash(header.hash()), hash)
					{
				...
				BlockNumber::Latest => {
					native_number = Some(header.number().clone().unique_saturated_into() as u32);
				}
...
impl<B, C, SC, P, CT, BE> EthApiT for EthApi<B, C, SC, P, CT, BE>
where
	C: ProvideRuntimeApi<B> + StorageProvider<B, BE>,
	C::Api: EthereumRuntimeApi<B>,
	BE: Backend<B> + 'static,
	...
	CT: ConvertTransaction<<B as BlockT>::Extrinsic> + Send + Sync + 'static,
{
	...
	fn chain_id(&self) -> Result<Option<U64>> {
		...
		Ok(Some(
			self.client
				.runtime_api()
				.chain_id(&BlockId::Hash(header.hash()))
				.map_err(|_| internal_err("fetch runtime chain id failed"))?
				.into(),
		))
	}
	...
	fn block_by_hash(&self, hash: H256, full: bool) -> Result<Option<RichBlock>> {
		...
		if let Ok((Some(block), statuses)) = self
			.client
			.runtime_api()
			.block_by_hash_with_statuses(&BlockId::Hash(header.hash()), hash)
		{
	...
	fn block_by_number(&self, number: BlockNumber, full: bool) -> Result<Option<RichBlock>> {
		...
		if let Ok(Some(native_number)) = self.native_block_number(Some(number)) {
			if let Ok((Some(block), statuses)) = self
				.client
				.runtime_api()
				.block_by_number(&BlockId::Hash(header.hash()), native_number)
			{
	...
	fn transaction_by_hash(&self, hash: H256) -> Result<Option<Transaction>> {
		...
		if let Ok(Some((transaction, block, status, _receipt))) = self
			.client
			.runtime_api()
			.transaction_by_hash(&BlockId::Hash(header.hash()), hash)
		{
			return Ok(Some(transaction_build(transaction, block, status)));
		}
		Ok(None)
	}
	...
	fn transaction_by_block_hash_and_index(
		...
		if let Ok(Some((transaction, block, status))) = self
			.client
			.runtime_api()
	...
	fn transaction_receipt(&self, hash: H256) -> Result<Option<Receipt>> {
		let header = self
			...
			.best_chain()
			.map_err(|_| internal_err("fetch header failed"))?;
		if let Ok(Some((_transaction, block, status, receipts))) = self
			.client
			.runtime_api()
			.transaction_by_hash(&BlockId::Hash(header.hash()), hash)
		{
	...
	fn compile_solidity(&self, _: String) -> Result<Bytes> {
		Err(not_supported_err(
			"Method eth_compileSolidity not supported.",
		))
	}

	fn compile_serpent(&self, _: String) -> Result<Bytes> {
		Err(not_supported_err(
			"Method eth_compileSerpent not supported.",
		))
	}
	...
	fn logs(&self, filter: Filter) -> Result<Vec<Log>> {
		...
		if let Ok(logs) = self.client.runtime_api().logs(
			&BlockId::Hash(header.hash()),
			from_block,
			to_block,
			filter.block_hash,
